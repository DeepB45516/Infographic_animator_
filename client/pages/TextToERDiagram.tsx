...
const CleanERDiagram: React.FC<{
  ...
  useEffect(() => {
    ...
    const entitySpacingX = 400; // Horizontal spacing between entities
    const entitySpacingY = 350; // Vertical spacing between entities
    const startX = 150; // Starting X position for the grid
    const startY = 150; // Starting Y position for the grid
    ...
    relationships.forEach((relationship) => {
      ...
      if (fromPos && toPos) {
        const midX = (fromPos.x + toPos.x) / 2 + 120; // Offset X
        const midY = (fromPos.y + toPos.y) / 2 + 30;  // Offset Y
  ...
  const drawCleanRelationship = useCallback((rel: ERRelationship) => {
    ...
    const relPos = relationshipPositions[rel.id] || { 
      x: (fromPos.x + toPos.x) / 2 + 60, // Default midpoint with offset
